...
function fieldLedger(c){const groups=[{name:'Cluster',fields:c.completeness.clusterFields},...c.parts.map(p=>({name:p.name,fields:c.completeness.nodeFields[p.name]||[]}))];
 ...
 return '<details class="ledger"><summary aria-label="'+present+' of '+total+' tracked fields are documented. Open the full list."><span class="ledgerfigure">'+present+' / '+total+'</span> <span class="ledgerlabel">documented fields <em>· view all '+total+'</em></span><span class="chevron" aria-hidden="true">⌄</span></summary><div class="ledgerbody"><p class="specnote">Every field this catalog checks for '+esc(c.name)+'. The sheets above list the fields that have a value; the rest are listed here. A field marked absent is not publicly available in the sources we could verify, which does not mean the hardware is missing. Fields that cannot apply to this system, such as benchmark results for an unranked cluster, are excluded from the total.</p>'+groups.map(g=>{const scored=g.fields.filter(f=>f.applicable!==false);return '<div class="ledgergroup"><h4>'+esc(g.name)+' <small>'+scored.filter(f=>f.present).length+' / '+scored.length+'</small></h4><ul>'+g.fields.map(cell).join('')+'</ul></div>'}).join('')+'</div></details>'}
...
function unknownNetwork(){
 ...
 $('#visual').innerHTML='<div class="unknown-network"><span class="pill">TOPOLOGY NOT PUBLISHED</span><h2>Known hardware. Undocumented layout.</h2><p>No switch groups or node-to-node paths are drawn because their arrangement is not publicly available.</p><button class="unknown-fabric" data-info="network"><span class="unknown-mark" aria-hidden="true">?</span><span><small>CLUSTER INTERCONNECT · SELECT FOR DETAILS</small><strong>'+esc(cluster.network)+'</strong><em>Switch layout and cable routes not published</em></span></button><div class="unknown-inventory"><div><small>DOCUMENTED NODE INVENTORY</small><strong>'+(p.count?fmt(p.count)+' nodes':'Node count not published')+'</strong><p>'+esc(p.name)+'</p></div><button class="unknown-node" data-action="inside"><span aria-hidden="true">▣</span><strong>Representative node</strong><small>Explore CPUs, accelerators and memory ↗</small></button></div><p class="subnote">The dashed boundary marks missing layout information, not a physical network shape.</p></div>';
}
