...
function inspect(preview=false){const p=part();const isNetwork=view==='network'&&!preview;let content='<h2>'+ (isNetwork?'The connected system':'Node '+String(selected+1).padStart(5,'0'))+'</h2><span class="pill">'+esc(p.name)+'</span><p>'+(isNetwork?'Start with the network. Open a group to see its nodes, then explore the hardware inside.':'Representative '+esc(p.name)+' configuration. Select a chip or link to learn what it does.')+'</p>';
 ...
 else content+='<div class="specs">'+spec('▣',p.apu?'CPU INSIDE EACH APU':'PROCESSORS',(p.cpus==null?'Count not verified':p.cpus+' ×')+' '+p.cpu)+spec('▥',p.apu?'INTEGRATED ACCELERATORS':'ACCELERATORS',p.gpus?p.gpus+' × '+p.gpu:'No GPU','green')+spec('▤','HOST / UNIFIED MEMORY',p.ram)+ (p.gpus?spec('▤','ACCELERATOR MEMORY',p.vram,'green'):'')+spec('▱','LOCAL STORAGE',p.disk,'purple')+'</div>'+((view==='nodes'||preview)?'<button class="primary" data-action="inside">Open node diagram ↗</button>':'');
 
 content+='<p class="subnote">Source-backed profile · checked '+catalogDate+'</p><a href="'+cluster.sources[cluster.sources.length-1].url+'" target="_blank" rel="noopener noreferrer">Open operator / ranking source ↗</a>';
...
function info(type){const p=part();const infos={
 ...
 gpu:[p.apu?'APU · integrated CPU + GPU':'GPU · parallel accelerator',p.gpu+'. '+(p.apu?'CPU and GPU share the memory on each APU.':p.vram+'. Runs many parallel operations.')+(p.mixed?' This node has 8 MI100 devices and 1 MI210; the last symbol represents MI210.':'')],
...
function inside(){ $('#visual').onwheel=null;const p=part();let s=svgOpen('nodesvg','0 0 620 355');const gpus=p.gpus,cpus=p.cpus||1,apu=p.apu,paired=p.paired;
