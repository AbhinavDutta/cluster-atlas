...
const shown=v=>v==null||v===''?'Not published':v===true?'Yes':v===false?'No':typeof v==='number'?v.toLocaleString('en-US'):String(v);
const CLUSTER_FIELDS=[['site','Operator / site'],['country','Country'],['profileScope','Profile scope'],['manufacturer','Manufacturer'],['os','Operating system'],['network','Cluster interconnect'],['topology','Network topology'],['cores','Ranked core count'],['rmax','HPL Rmax (PFlop/s)'],['rpeak','Theoretical peak Rpeak (PFlop/s)'],['powerKw','Measured power (kW)'],['efficiency','Energy efficiency (GFlop/s per watt)'],['checked','Data checked']];
const PART_FIELDS=[['systemModel','System / node model'],['count','Compute nodes of this type'],['cpu','CPU model'],['cpus','CPU sockets per node'],['coresPerSocket','Cores per socket'],['physicalCores','Physical CPU cores'],['cpuNote','CPU note'],['gpu','Accelerator model'],['gpus','Physical accelerators per node'],['ram','Host / unified memory'],['vram','Accelerator memory'],['link','Accelerator ↔ accelerator link'],['onNodeFabric','On-node fabric'],['hostLink','CPU ↔ accelerator link'],['nic','Network interfaces per node'],['nicModel','Interface model'],['nicSpeed','Interface speed'],['nicTopology','Interface-to-accelerator topology'],['aggregateScaleOut','Aggregate scale-out per node'],['gpuDirect','GPUDirect RDMA'],['managementNic','Management / storage networking'],['disk','Local storage'],['nvme','NVMe data disks'],['remoteDisks','Max remote data disks'],['diskIops','Uncached disk IOPS'],['diskThroughputMBps','Uncached disk throughput (MBps)'],['formFactor','Form factor']];
const SKIP=new Set(['name','parts','sources','notes','completeness','platform','benchmark','aliases','hardwareDescription','rank','profileScope','checked','site','country','network','topology','cores','rmax','rpeak','powerKw','efficiency','platformFacts','scaleOut','compact','apu','paired','mixed','noGpuLinks','groupSize','acceleratorLabel','id']);
function rows(record,fields){const seen=new Set();const out=[];
 for(const [key,label] of fields){seen.add(key);out.push([label,shown(record[key])]);}
 for(const key of Object.keys(record)){if(seen.has(key)||SKIP.has(key))continue;const v=record[key];if(v==null||typeof v==='object')continue;out.push([key.replace(/([A-Z])/g,' $1').replace(/^./,c=>c.toUpperCase()),shown(v)]);}
 return out;}
function sheet(title,rows_,note=''){return '<section class="specblock"><h3>'+esc(title)+'</h3>'+(note?'<p class="specnote">'+esc(note)+'</p>':'')+'<dl>'+rows_.map(([k,v])=>'<div'+(v==='Not published'?' class="unpublished"':'')+'><dt>'+esc(k)+'</dt><dd>'+esc(v)+'</dd></div>').join('')+'</dl></section>'}
function benchmarkStrip(c){const b=c.benchmark;if(!b)return '';const items=[['HPL Rmax',b.rmax?b.rmax.toLocaleString('en-US')+' PFlop/s':null],['Theoretical peak',b.rpeak?b.rpeak.toLocaleString('en-US')+' PFlop/s':null],['Ranked cores',b.cores?b.cores.toLocaleString('en-US'):null],['Measured power',b.powerKw?b.powerKw.toLocaleString('en-US')+' kW':null],['Energy efficiency',b.efficiencyGFlopsPerWatt?b.efficiencyGFlopsPerWatt.toFixed(2)+' GFlop/s·W':null],['TOP500 rank','#'+b.rank]];return '<div class="benchmark">'+items.map(([label,value])=>'<div><small>'+label+'</small><strong'+(value?'':' class="mutedvalue"')+'>'+(value||'Not published')+'</strong></div>').join('')+'</div>'}
function specificationView(){const c=cluster,p=part();const missing=c.completeness?.missing||[];
 return '<div class="specsheet"><div class="spechead"><div><span class="pill">FULL PUBLIC RECORD</span><h2>'+esc(c.name)+' specifications</h2><p>Every fact this catalog holds for '+esc(c.name)+', including fields that are not part of a diagram. Values the operator has not published are marked rather than estimated.</p></div><div class="completeness" role="status"><strong>'+c.completeness.documented+' / '+c.completeness.total+'</strong><small>documented fields</small><div class="meter" aria-hidden="true"><span style="width:'+c.completeness.percent+'%"></span></div></div></div>'
 +benchmarkStrip(c)
 +sheet('Cluster · '+p.name,rows(c,CLUSTER_FIELDS))
 +c.parts.map(pp=>sheet(pp.name+(pp===p?' · selected node type':'')+'',rows(pp,PART_FIELDS),pp.platformFacts?'Fields marked from the documented platform specification: '+pp.platformFacts.join(', ')+'.':'')).join('')
 +sheet('Scope, caveats and sources',[['Profile scope',c.profileScope||'Documented compute inventory'],['Catalog notes',c.notes],['Node types',c.parts.length],['Sources',c.sources.map(s=>s.title+' — '+s.url).join(' | ')]])
 +'<section class="specblock"><h3>Not published for this system</h3><p class="specnote">These fields are absent from the public record this catalog could verify. A missing value does not mean the hardware is absent.</p><ul class="gapList">'+(missing.length+Object.values(c.completeness?.nodeMissing||{}).flat().length?'': '<li>No gaps recorded.</li>')+missing.map(m=>'<li><strong>Cluster:</strong> '+esc(m)+'</li>').join('')+Object.entries(c.completeness?.nodeMissing||{}).flatMap(([n,list])=>list.map(m=>'<li><strong>'+esc(n)+':</strong> '+esc(m)+'</li>')).join('')+'</ul></section></div>';}
