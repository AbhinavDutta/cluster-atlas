...
export function initWorldMap(clusters, onSelect) {
  ...
  root.innerHTML=`<details class="world-discovery"><summary><span><span class="eyebrow">GLOBAL ATLAS</span><strong>Explore the world of compute</strong></span><span class="map-summary">${mapped.length} individual pins · ${unknown.length} not pinned <span class="map-chevron">⌄</span></span></summary>
    ...
    <div class="world-credit">One pin per cluster · Thin lines connect offset pins to their city<br>Land: <a href="https://www.naturalearthdata.com/about/terms-of-use/" target="_blank" rel="noopener noreferrer">Natural Earth</a> · Scroll or pinch to zoom · Drag to pan<br>Only sourced city locations are pinned; unmapped locations stay in the list.</div></div>
    <aside class="world-panel" aria-label="Clusters at selected map location"><div class="world-results" aria-live="polite"></div>${unknown.length?`<div class="world-unknown"><strong>Location not mapped</strong><p>City not recorded in this catalog, or the system spans multiple sites.</p><div class="world-unmapped-list" tabindex="0" role="region" aria-label="Unmapped clusters; scroll for more">${unknown.map(c=>`<button title="${escape(c.name)}" data-map-cluster="${c.id}">${escape(c.name)} <span>Explore ↗</span></button>`).join('')}</div></div>`:''}</aside></div></details>`;
