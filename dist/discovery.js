...
export function matchingParts(c,f){
 ...
 return c.parts.map((p,index)=>({p,index})).filter(({p})=>(!f.model||acceleratorModels(p).includes(f.model))&&(!f.cpu||cpuArchitecture(p)===f.cpu)&&(!f.memory||(gpuMemoryGB(p)!=null&&gpuMemoryGB(p)>=Number(f.memory))));
}
export function initDiscovery(clusters,onOpen,onCompare){
 ...
 const models=[...new Set(clusters.flatMap(c=>c.parts.flatMap(acceleratorModels)).filter(Boolean))].sort();
 root.innerHTML=`<details class="discovery"><summary><span><span class="eyebrow">FIND YOUR HARDWARE</span><strong>Discover clusters by requirements</strong></span><span>Filter catalog ⌄</span></summary><div class="discovery-body"><form id="hardwareFilters"><div class="hardware-filters">${select('model','GPU / accelerator model',models)}${select('memory','Minimum memory per GPU',[['32','32 GB'],['40','40 GB'],['80','80 GB'],['96','96 GB'],['128','128 GB'],['141','141 GB']])}${select('cpu','CPU architecture',['x86-64','Arm','Sunway'])}${select('network','Cluster interconnect',[...new Set(clusters.map(networkFamily).filter(Boolean))].sort())}${select('access','Research access',[['research','Documented application route'],'EuroHPC','NERSC / ERCAP','NSF ACCESS'])}</div><div class="discovery-actions"><button type="button" data-filter-preset>Try: 80 GB GPUs + research access</button><button type="reset">Clear filters</button></div></form><p class="discovery-note">Model filters group documented variants under one canonical model; the original specification is shown below. Architecture-only labels (such as Hopper) do not qualify as a specific model. Requirements must match the same node type. Unknown specifications do not qualify. GPU memory excludes shared APU memory, MI250X package totals (two GPU dies), and ambiguous variants; GiB is converted to GB for filtering. Hardware matches do not guarantee workload performance or access.</p><div class="discovery-count" role="status" aria-live="polite"></div><div class="discovery-results" role="region" aria-label="Matching clusters" tabindex="0"></div></div></details>`;
