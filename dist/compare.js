...
export function initComparison(clusters,getCurrent){
  ...
  function paint(){
    ...
    tbody.innerHTML=section('Whole-cluster inventory')+
      clusterRow('Operator / site',c=>c.site)+clusterRow('Country',c=>c.country)+
      clusterRow('Total compute nodes',c=>fmt(sum(c,p=>p.count)))+
      clusterRow('Total physical GPUs / APUs',c=>fmt(sum(c,p=>p.gpus===0?0:p.count==null?null:p.count*p.gpus)))+
      ...
      nodeRow('Accelerator model',p=>p.gpus?p.gpu:'No GPU')+nodeRow('Physical accelerators',p=>`${p.gpus}${p.apu?' APUs (CPU + GPU)':''}`)+
      nodeRow('Host / unified memory',p=>p.ram)+nodeRow('Accelerator memory (as documented)',p=>p.gpus?p.vram:'Not applicable')+
      nodeRow('Storage (as documented)',p=>p.disk)+
      section('Connections')+nodeRow('CPU ↔ accelerator',p=>p.gpus?p.hostLink:'Not applicable')+
      nodeRow('Accelerator ↔ accelerator',p=>p.gpus>1?p.link:'Not applicable')+
  ...
  root.addEventListener('click',e=>{const b=e.target.closest('[data-component]');if(!b)return;const i=Number(b.dataset.side),s=sides[i],p=s.c.parts[s.p];const values={cpu:p.cpu,gpu:p.gpu+(p.mixed?' · 8 MI100 devices and 1 MI210.':'')+(p.apu?' · CPU and GPU share package memory.':''),ram:p.ram+(p.apu?' · Unified memory; do not add accelerator memory again.':''),host:p.hostLink,local:p.link,network:s.c.network,disk:p.disk};root.querySelector('.compare-detail').textContent=`${i?'Right':'Left'} · ${s.c.name} · ${values[b.dataset.component]??'Not verified'}`;});
