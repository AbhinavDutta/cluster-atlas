...
const eurohpc=src('EuroHPC JU · Our supercomputers','https://eurohpc-ju.europa.eu/supercomputers/our-supercomputers_en');
const microsoftH200=src('Microsoft · ND H200 v5 VM series','https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/gpu-accelerated/ndh200v5-series');
const dcai=src('Danish Centre for AI Innovation · Gefion','https://www.dcai.dk/gefion');
const superpod=src('NVIDIA · DGX SuperPOD','https://www.nvidia.com/en-us/data-center/dgx-superpod/');
const topList=src('TOP500 · June 2026 list','https://top500.org/lists/top500/list/2026/06/');
export const extraClusters=[
 ...
 ],[src('Meta · RSC first-phase architecture','https://ai.meta.com/blog/ai-rsc/'),dgxa],'January 2022 first-phase snapshot: 760 DGX A100 systems / 6,080 GPUs. Later expansion plans are not counted as installed hardware. CPU and on-node links follow the documented DGX A100 platform; installed memory variant is not published.',{aliases:['rsc','meta rsc','research supercluster'],profileScope:'January 2022 first-phase snapshot'}),
 C('rainier','Project Rainier','AWS / Anthropic','United States','AWS Elastic Fabric Adapter (EFAv3) · 3.2 Tb/s per Trn2 instance',[
 P('Trn2 instance · representative building block',null,'Host CPU model not published · 192 vCPUs per instance',null,'AWS Trainium2',16,'2 TB host memory (instance specification)','1.5 TB HBM3 across 16 chips','NeuronLink',{disk:'4 × 1.92 TB NVMe (instance specification)',acceleratorLabel:'AI chip',compact:true})
 ...
 C('reindeer','Reindeer','Microsoft Azure','United States','NVIDIA InfiniBand NDR',[
 P('ND H200 v5',null,'Intel Xeon Platinum 8480C (Sapphire Rapids)',null,'NVIDIA H200',8,'1,850 GiB (VM specification)','141 GB per GPU','NVLink 4 · all GPU pairs',{
 ...
 ],[microsoftH200,topList],'The node model uses public ND H200 v5 VM specifications. TOP500 ranks this system #57 (June 2026) with a Rmax of 45.59 PFlop/s. Exact benchmark VM count, the physical host socket allocation behind the 96 reported vCPUs, and the scale-set topology are not published.',{rank:57,rmax:45.59,aliases:['azure reindeer','nd h200 v5'],profileScope:'Ranked system / partition'})
];
...
export function enrichRanked(c){
 ...
 if(c.rank===16){
  add(P('ACC compute',1120,'Intel Xeon Platinum 8460Y+ · 40 cores',2,'NVIDIA H100',4,'512 GB DDR5','64 GB HBM2 per GPU','Not verified',{
   hostLink:'PCIe 5.0',
   nic:4,
   nicModel:'NVIDIA ConnectX-7 NDR200 InfiniBand',
   nicSpeed:'200 Gb/s per adapter · 800 Gb/s per node',
   disk:'480 GB NVMe (/scratch)'
  }),'https://www.bsc.es/supportkc/docs/MareNostrum5/overview','Accelerated partition (ACC) only; the 6,408-node general-purpose partition, the GPP-HBM nodes and the AI upgrade partitions are outside this profile. BSC documents a three-level fat-tree with 324 QM9790 switches across seven ACC islands. The machine-wide storage totals 248 PB of SSD/flash plus disk and a 402 PB tape archive.');
  c.topology='Fat-tree (3-level InfiniBand)';
  c.sources.push(src('BSC · MareNostrum 5 technical information','https://www.bsc.es/marenostrum/marenostrum-5'));
 }
 if(c.rank===42){
  add(P('GPU compute',382,'NVIDIA Grace · 72 Arm Neoverse V2 cores per superchip',4,'NVIDIA GH200 Superchip',4,'128 GB LPDDR5X per module (4 modules per node)','96 GB HBM3 per GPU','Not verified',{
   hostLink:'NVLink-C2C · 900 GB/s coherent CPU ↔ GPU',
   nic:4,
   nicModel:'HPE Slingshot 11',
   nicSpeed:'200 Gb/s per adapter',
   disk:'1.8 TB NVMe node-local scratch'
  }),'https://www.naiss.se/resources/arrhenius-technical-description/','Arrhenius GPU partition only: 382 nodes, each with four Grace Hopper superchips. The 424-CPU HPC partition, the SENS sensitive-data partition and the PCD partition are excluded. CPU and GPU partitions share a Lustre filesystem of 25 PB disk (260 GiB/s) plus 2 PB flash (15M IOPS).');
  c.sources.push(src('NAISS · Arrhenius','https://www.naiss.se/arrhenius'));
 }
 ...
 if(c.rank===19)add(P('H200 compute',766,'Intel Xeon Platinum 8558 · 48 cores',2,'NVIDIA H200',8,'2 TB DDR5','141 GB HBM3e per GPU','Not verified',{nic:8,disk:'2 × 7.68 TB NVMe'}),'https://abci.ai/en/about_abci/computing_resource.html','766 compute nodes. Eight NDR200 compute-network ports per node; a separate HDR connection serves storage. Local GPU and host wiring are not published in this profile.');
 if(c.parts[0].name.startsWith('NVIDIA DGX H100')){
  Object.assign(c.parts[0],{name:'DGX H100 platform',cpus:2,gpus:8,ram:'2 TB (DGX H100 platform specification)',vram:'80 GB per GPU',link:'NVLink / NVSwitch',hostLink:'PCIe 5.0'});c.sources.push(dgx);c.notes+=' Per-node CPU/GPU counts, RAM and local links use NVIDIA DGX H100 platform specifications; operator-specific modifications are not published.';
 }
 if(c.rank===45){Object.assign(c.parts[0],{name:'DGX A100 platform',cpus:2,gpus:8,link:'NVLink / NVSwitch',hostLink:'PCIe 4.0'});c.sources.push(dgxa);c.notes+=' DGX A100 node counts and local links follow NVIDIA platform documentation; installed memory variant is not published.';}
 if(c.rank===31){
  // EuroHPC and GRNET publish the architecture but not a node count or a
  // superchips-per-node figure, so count, sockets and per-superchip memory are
  // left unprompted rather than derived from the ranking core count.
  Object.assign(c.parts[0],{
   name:'GH200 compute',
   cpu:'NVIDIA Grace · 72 Arm Neoverse V2 cores',
   gpu:'NVIDIA GH200 Superchip',
   cpuNote:'DAEDALUS is partitioned by core: the CPU partition uses 64 of each GH200 superchip’s 72 Arm cores, while the accelerated partition uses the remaining 8 cores together with the H100.',
   hostLink:'NVLink-C2C · 900 GB/s coherent CPU ↔ GPU'
  });
  c.sources.push(eurohpc);
  c.notes+=' EuroHPC describes DAEDALUS as an HPE NVIDIA GH200 direct-liquid-cooled system with 1 PB of high-performance NVMe plus 10 PB of usable storage capacity. The number of superchips per node, the total node count and the H200/H100 memory variant are not published.';
 }
 if(c.rank===43){
  c.sources.push(dcai,superpod);
  c.notes+=' The Danish Centre for AI Innovation describes Gefion as a large-scale NVIDIA DGX SuperPOD with more than 1,540 GPUs mixing NVIDIA DGX H100 and NVIDIA B300 systems, alongside 110 PB of WEKA storage. The TOP500 entry ranks the DGX H100 component; the DGX H100 node layout above is the vendor platform specification.';
 }
 return c;
}
